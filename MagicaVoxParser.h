...
public:
   static void LoadModel(const std::string &fileName, VoxModel &model)
   {
      ...
      if (header.version != 150) {
         std::cerr << "Wrong voxel file format" << std::endl;
      }
      ...
      bool hasColorPallete = false;
      VoxChunk chunk;
      while (file.read(reinterpret_cast<char *>(&chunk), sizeof(VoxChunk))) {
         LogChunk(chunk);
         ...
         if (id == "SIZE") {
            ...
            hasColorPallete = true;
            size_t bytesRead = 0;
            int color;
            while (bytesRead < chunk.chunkBytes && file.read(reinterpret_cast<char *>(&color), sizeof(int))) {
               bytesRead += file.gcount();
...
private:
   static void LogChunk(VoxChunk &chunk)
   {
      std::cout << "ID: " << chunk.id << " bytes: " << chunk.chunkBytes << " children bytes: " << chunk.childrenBytes << std::endl;
   }

   static std::vector<unsigned int> DefaultPallete()
   {
